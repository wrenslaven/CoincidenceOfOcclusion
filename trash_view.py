...
class TrashView:
    ...
    def load_trash_view(self, event):
        self.controller.gamestate = "trash"
        ...
        self.load_screenshot_icons_view(event=None)
    ...
    def load_screenshot_icons_view(self, event):
        ...
        self.icon_photoimage_dict = assetloader.load_icons("trashed_screenshots")
    ...
    def load_screenshot(self, image_name, event):
        ...
        image = assetloader.load_single_image("trashed_screenshots", image_name)
        ...
        trash_button = tk.Button(self.root, text="Recover", command=lambda: self.recover_from_trash(image_name))
        self.canvas.create_window(645, 510, window=trash_button)

    def recover_from_trash(self, image_name):
        source_path = f"trashed_screenshots/{image_name}"
        destination_path = f"screenshots/{image_name}"
